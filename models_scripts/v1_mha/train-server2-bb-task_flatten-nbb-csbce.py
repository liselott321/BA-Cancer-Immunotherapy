...
# Loss
class CostSensitiveBCE(nn.Module):
    def __init__(self, fp_cost=5.0, fn_cost=1.0, eps=1e-8):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        loss = -(
            self.fn_cost * targets * torch.log(probs + self.eps) +
            self.fp_cost * (1 - targets) * torch.log(1 - probs + self.eps)
        )
...
loss_fn = CostSensitiveBCE(fp_cost=2.0)
...
for epoch in range(epochs):
    ...
    model_save_dir = "results/trained_models/v1_mha/csbce/epochs"
