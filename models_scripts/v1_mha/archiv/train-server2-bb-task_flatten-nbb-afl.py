...
class AsymmetricFocalLoss(nn.Module):
    def __init__(self, gamma_pos=0.0, gamma_neg=5.0, alpha=0.35, eps=1e-8):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        pos_loss = -self.alpha * (1 - probs) ** self.gamma_pos * targets * torch.log(probs + self.eps)
        neg_loss = -(1 - self.alpha) * probs ** self.gamma_neg * (1 - targets) * torch.log(1 - probs + self.eps)
...
loss_fn = AsymmetricFocalLoss(gamma_pos=0.5, gamma_neg=2, alpha=0.35)
...
for epoch in range(epochs):
    ...
    model_save_dir = "results/trained_models/v1_mha/afl/epochs"
