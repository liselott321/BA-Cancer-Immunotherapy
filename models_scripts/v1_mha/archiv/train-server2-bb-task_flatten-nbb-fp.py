...
def false_positive_penalty_loss(logits, targets, fp_weight=3.0):
    probs = torch.sigmoid(logits)
    preds = (probs > 0.5).float()
    fp_mask = (targets == 0) & (preds == 1)
    penalty = fp_weight * probs[fp_mask]
    return penalty.mean() if fp_mask.any() else torch.tensor(0.0, device=logits.device)
...
for epoch in range(epochs):
    ...
    for tcr, epitope, label in train_loader_tqdm:
        ...
        loss += false_positive_penalty_loss(output, label)
    ...
    with torch.no_grad():
        ...
        fp_val_df = val_data.iloc[false_positive_indices].copy()
        fp_val_df["predicted_score"] = all_outputs[false_positive_indices]
        fp_val_df["predicted_label"] = all_preds[false_positive_indices]
            
        # Speichern pro Epoche
        os.makedirs("results/fp_per_epoch", exist_ok=True)
        fp_val_df.to_csv(f"results/fp_per_epoch/false_positives_epoch_{epoch+1}.csv", sep="\t", index=False)
