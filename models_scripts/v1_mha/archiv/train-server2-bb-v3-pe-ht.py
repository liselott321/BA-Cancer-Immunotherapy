...
dropout = config.get("dropout", wandb.config.dropout)
embed_dim = config.get("embed_dim", wandb.config.embed_dim)
classifier_hidden_dim = config.get("classifier_hidden_dim", wandb.config.classifier_hidden_dim)
optimizer_name = args.optimizer or wandb.config.get("optimizer", "adam")
...
for epoch in range(epochs):
    ...
    if ap > best_ap or val_loss_total < best_val_loss:
        best_ap = max(best_ap, ap)
        best_val_loss = min(best_val_loss, val_loss_total)
