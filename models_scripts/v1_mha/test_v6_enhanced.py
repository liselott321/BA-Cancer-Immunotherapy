...
run = wandb.init(
    ...
    job_type="test_model",
    name=f"Test_Run_v6_enhanded_epoch_{epoch_model}",
    config=config
)
...
artifact_name = f"ba_cancerimmunotherapy/dataset-allele/model_checkpoint_epoch_{epoch_model}:v0" #anpassen, wenn andere version latest oder v12
...
# If checkpoint is a dictionary with a 'model_state_dict' key, extract the state dict
if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
    model.load_state_dict(checkpoint['model_state_dict'])
    print(f"Loaded model from checkpoint at epoch {checkpoint.get('epoch', 'unknown')}")
else:
    # Try loading it directly (in case it's just a state dict)
    try:
