...
from models.morning_stars_v1.beta.v5_reciprocal_attention_cf_ple import TCR_Epitope_Transformer, LazyTCR_Epitope_Descriptor_Dataset
...
MODEL_NAME = "v5_reci_v2_over_new"
...
train_dataset = LazyTCR_Epitope_Descriptor_Dataset(train_data, tcr_train_embeddings, epitope_train_embeddings, ple_tcr_tensor, ple_epi_tensor, trbv_dict, trbj_dict, mhc_dict)
val_dataset = LazyTCR_Epitope_Descriptor_Dataset(val_data, tcr_valid_embeddings, epitope_valid_embeddings, ple_tcr_tensor, ple_epi_tensor, trbv_dict, trbj_dict, mhc_dict)
...
for epoch in range(epochs):
    ...
    # Create a new balanced dataloader for this epoch
    train_loader = balanced_generator.get_loader()
    train_loader_tqdm = tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} [Training]", leave=False)

    for tcr, epitope, tcr_ple, epi_ple, trbv, trbj, mhc, label in train_loader_tqdm:
        # Move data to device
        tcr, epitope, tcr_ple, epi_ple, trbv, trbj, mhc, label = (
            tcr.to(device),
            epitope.to(device),
            tcr_ple.to(device),
            epi_ple.to(device),
        ...
        optimizer.zero_grad()
        output = model(tcr, epitope, tcr_ple, epi_ple, trbv, trbj, mhc,)
    ...
    with torch.no_grad():
        for tcr, epitope, tcr_ple, epi_ple, trbv, trbj, mhc, label in val_loader_tqdm:
    ...
    model_save_dir = "results/trained_models/v5_reci_v2_over/epochs"
...
# Save best model -------------------------------------------------------------------------------
if best_model_state:
    os.makedirs("results/trained_models/v5_reci_v2_over_new", exist_ok=True)
