...
class TwoStageBottleneck(nn.Module):
    def __init__(self, in_dim, mid_dim, bottleneck_dim, dropout):
        super().__init__()
        self.reduce1 = nn.Linear(in_dim, mid_dim)
        self.act1    = nn.ReLU()
        self.drop1   = nn.Dropout(dropout)
        self.reduce2 = nn.Linear(mid_dim, bottleneck_dim)
        self.act2    = nn.ReLU()
        self.drop2   = nn.Dropout(dropout)

    def forward(self, x):
        x = self.drop1(self.act1(self.reduce1(x)))
        x = self.drop2(self.act2(self.reduce2(x)))
        return x
...
class Classifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, dropout):
        super().__init__()
        self.bottleneck = TwoStageBottleneck(input_dim, hidden_dim*2, hidden_dim, dropout)
        self.res        = ResidualBlock(hidden_dim, dropout)
...
class TCR_Epitope_Transformer(nn.Module):
    def __init__(self, embed_dim=1024, num_heads=4, num_layers=2, max_tcr_length=43, max_epitope_length=43, dropout=0.1, classifier_hidden_dim=64):
        super(TCR_Epitope_Transformer, self).__init__()
        # Wir nehmen embed_dim == 1024 direkt
        self.embed_dim = embed_dim

        # Keine Linear(1024→embed_dim) mehr!
        self.tcr_embedding     = nn.Identity()
        self.epitope_embedding = nn.Identity()
        ...
        # Klassifikator: Input ist (L_tcr + L_epi) * 1024
    ...
    def forward(self, tcr, epitope):
        # tcr: [B, L_tcr, 1024], epitope: [B, L_epi, 1024]
