...
class TCR_Epitope_Transformer_AllFeatures(nn.Module):
    def __init__(self, embed_dim=128, num_heads=4, num_layers=2,
        ...
        # NEW: Bidirectional Cross-Attention Blocks
        self.epitope_cross_attn_block = CrossAttentionBlock(embed_dim, num_heads, dropout)  # Epitope attends to TCR
        self.tcr_cross_attn_block = CrossAttentionBlock(embed_dim, num_heads, dropout)      # TCR attends to Epitope
    ...
    def forward(self, tcr, epitope, tcr_physchem, epi_physchem, trbv, trbj, mhc):
        ...
        cond_epitope = self.epitope_cross_attn_block(tcr_emb, epitope_emb, key_padding_mask=tcr_mask)

        # NEW: TCR attends to Epitope
        cond_tcr = self.tcr_cross_attn_block(epitope_emb, tcr_emb, key_padding_mask=epitope_mask)

        # Masked mean pooling
        tcr_emb_masked = cond_tcr.masked_fill(tcr_mask.unsqueeze(-1), 0)
