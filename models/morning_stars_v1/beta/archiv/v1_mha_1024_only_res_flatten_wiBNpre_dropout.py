...
class ResidualBlock(nn.Module):
    def __init__(self, hidden_dim, res_dropout):
...
class Classifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, res_dropout):
...
class AttentionBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, attn_dropout=0.1, ffn_dropout=0.1):
...
class TCR_Epitope_Transformer(nn.Module):
    def __init__(self, attn_dropout, ffn_dropout, res_dropout, embed_dim=128, num_heads=4, num_layers=2, max_tcr_length=43, max_epitope_length=43, classifier_hidden_dim=64):
        ...
        self.transformer_layers = nn.ModuleList([
            AttentionBlock(embed_dim, num_heads, attn_dropout, ffn_dropout) for _ in range(num_layers)
        ])
        ...
        self.classifier = Classifier(self.classifier_input_dim, classifier_hidden_dim, res_dropout)
