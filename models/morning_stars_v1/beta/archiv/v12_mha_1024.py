...
class AttentionBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        self.norm2 = nn.LayerNorm(embed_dim)  # Pre-LN for FFN
    ...
    def forward(self, x, key_padding_mask=None):
        ...
        attn_output, _ = self.attn(
            x_norm.permute(1, 0, 2),  # Query
            x_norm.permute(1, 0, 2),  # Key
            x_norm.permute(1, 0, 2),  # Value
            key_padding_mask=key_padding_mask
        )
        x = x + self.dropout(attn_output.permute(1, 0, 2))  # Residual Connection

        # Pre-LN before Feed-Forward Network (FFN)
        x_norm = self.norm2(x)
        x = x + self.dropout(self.ffn(x_norm))  # Residual Connection with FFN
...
#     ## 
#     # x = x + self.dropout(self.ffn(self.norm1(x)))  # Apply FFN after attention
